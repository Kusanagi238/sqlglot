...
class Validator(unittest.TestCase):
    ...
    def validate_all(self, sql, read=None, write=None, pretty=False, identify=False):
        ...
        # Normalize `read` and `write` so callers may pass either a mapping or
        # a single SQL string. This prevents calling .items() on non-mapping
        # objects (e.g. a plain string) which would raise AttributeError.
        if read is None:
            read = {}
        elif isinstance(read, str):
            read = {self.dialect: read}
        elif not hasattr(read, "items"):
            raise TypeError(f"read must be a mapping or string, got {type(read)!r}")

        if write is None:
            write = {}
        elif isinstance(write, str):
            write = {self.dialect: write}
        elif not hasattr(write, "items"):
            raise TypeError(f"write must be a mapping or string, got {type(write)!r}")
        ...
        for write_dialect, write_sql in write.items():
